...
(defclass! phoneme (class-array)
   ((transition :initform nil :accessor transition))
   (:icon 150)
...
(defmethod get-slot-in-out-names ((self phoneme))
   (values '("self" "numcols" "transition") '(nil 1 nil)
           '("phoneme object" "number of elements (formants)" "transition curve (BPF)")
           '(nil nil nil)))

(defmethod fixed-slots-list ((self phoneme)) '(numcols transition))

;;; creation of a default transition with N formants
(defun default-phoneme (n)
  (om-make-array 'phoneme n (om-make-bpf 'bpf '(0 1) '(0 1) 3)
...
(defun def-phonemes-dictionary ()
  (or *default-phonemes*
      (setf *default-phonemes*
            (list 
             (list "b" 
                   ;(create-list 5 (om-make-bpf 'bpf '(0.025 0.2 0.5 0.7 1) '(1 0.5 0 0 0.3 1) 3))
                   ;(create-list 5 (om-make-bpf 'bpf '(0.02 0.15 0.85 0.92 1) '(1 0.1 0 0 0.05 1) 3))
                   ;(create-list 5 (om-make-bpf 'bpf '(0 1) '(1 1) 3))
                   ;(om-make-bpf 'bpf '(0 1) '(0 1) 3)
                   (om-make-array 'phoneme 5 (om-make-bpf 'bpf '(0 1) '(0 1) 3)
...
(defmethod! make-fof (db-entry onset dur)
  ...
  (multiple-value-bind (fofvalues freqs amps bws)
    ...
    (when fofvalues
      (om-make-array 'ch-fof (length fofvalues) onset dur nil nil
          ...
          (if (= dur 0)
            ...
            (let* ((x-list (third (multiple-value-list (om-sample (list 0 dur) ctrl)))))
              (om-make-array 'ch-fof (max (slot-value fof1 'numcols) (slot-value fof2 'numcols))
                             ...
                             nil 
                             nil
                             :freq (make-bpfs x-list freq-curves (inter-trans (get-array-row fof1 'freq) (get-array-row fof2 'freq) morph-curve ctrl) ctrl)
                             :amp (make-bpfs x-list amp-curves (inter-trans (get-array-row fof1 'amp) (get-array-row fof2 'amp) morph-curve ctrl) ctrl)
                             :bw (make-bpfs x-list bw-curves (inter-trans (get-array-row fof1 'bw) (get-array-row fof2 'bw) morph-curve ctrl) ctrl))))
          ))
...
<morph-curve> is the global trajectory of the interpolation.
    ...
    (append
     ...
     (loop for fof2 in fofsbis                                        
           ...
           (let* ((beg2 (get-absolute-time fof2))
             ...
             (setf slots1 (list (freq fof1)(amp fof1)(bw fof1)))
